...
int main(void) {
    ...
    const char *color = get_color_code(cfg.color);
    ...
    get_memory_info(info[6].value, BUFFER_SIZE, cfg.memory_unit);
    ...
    printf("%s%s %s%s%s@%sarchlinux\n" COLOR_RESET, color, logo[0], color, user, COLOR_RESET, color);
    ...
    for (int i = 0; i < INFO_COUNT; i++) {
        int logo_idx = i + 2;
        const char *logo_line = (logo_idx < LOGO_LINES) ? logo[logo_idx] : "                ";
        printf("%s%s " "%s%-*s" COLOR_RESET " : %s\n", 
               color, logo_line, 
               color, (int)max_key_len, info[i].key, 
               info[i].value);
